#!/usr/bin/env bash

function show_help() {
  echo "Usage: $0 <path_to_film> [PRESET] [CRF] [GRAIN]"
  echo "  <path_to_film>      Directory containing the input files"
  echo "  [PRESET]             Encoding compression preset (default: 2)"
  echo "  [CRF]                Constant Rate Factor (default: 12)"
  echo "  [GRAIN]              Film Grain Synthesis (default: 10)"
}

if [ "$1" == "--help" ] || [ "$1" == "-h" ] || [ -z "$1" ]; then
  show_help
  exit 0
fi

################################################################################

# The directory containing the film files
FILM_PATH="$1"

# Extract the base path and film name from the provided file path
BASE_DIRECTORY=$(dirname "$FILM_PATH")
FILM_NAME=$(basename "$FILM_PATH" .mkv)

# Check if both input and output directories are provided
if [ -z "$FILM_PATH" ] || [ -z "$BASE_DIRECTORY" ] || [ -z "$FILM_NAME" ]; then
  show_help
  exit 1
fi

# Encoding compression preset
# (lower value = better quality compression = longer encode times)
# ~2 if encode speed is not a consideration, otherwise ~5 for decent encode time and decent quality preservation
PRESET=${3:-2}
# Constant Rate Factor
# (lower values = better quality = bigger file size)
# ~14 for high quality source to preserve most of the original detail
CRF=${4:-12}
# Film Grain Synthesis
# (lower value = less grain = less perceived detail, depending on source)
# ~10 for low grain (e.g. Better Call Saul), ~15 for high grain (e.g. Chernobyl)
GRAIN=${5:-10}

############################################################

output_file="${BASE_DIRECTORY}/${FILM_NAME} (AV1).mkv"

if [ -f "$FILM_PATH" ]; then
  # Run the ffmpeg command to re-encode the video
  echo "reencoding \"${FILM_PATH}\" to \"${output_file}\""
  ffmpeg -hide_banner -loglevel warning -i "${FILM_PATH}" -map 0 -c:a copy -c:s copy -c:v libsvtav1 -crf $CRF -preset $PRESET -pix_fmt yuv420p10le -svtav1-params tune=0:film-grain=$GRAIN:film-grain-denoise=0 "${output_file}"
fi
